...
import { LogOut, User, Home, Plus, Menu, X, Sparkles, Bell } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6">
        <div className={`relative flex items-center justify-between transition-all duration-500 rounded-[2rem] px-6 h-16 border border-white/5 backdrop-blur-2xl ${isScrolled ? 'bg-black/60 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)]' : 'bg-white/5'
          }`}>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 mr-2">
              <button className="p-3 rounded-xl bg-white/5 border border-white/10 text-white/40 hover:text-[#80ff00] hover:bg-white/10 transition-all">
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 p-1.5 pr-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all group">
                  ...
                  <div className="hidden md:flex flex-col items-start translate-y-[-1px]">
                    ...
                    <span className="text-[8px] font-black text-white/30 uppercase tracking-widest leading-none">
                      Active Vitals
                    </span>
                  </div>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-64 bg-[#08080a] border-white/10 rounded-[2rem] p-4 shadow-2xl backdrop-blur-xl">
                ...
                <DropdownMenuItem asChild>
                  <Link href="/profile" className="flex items-center gap-4 p-4 rounded-2xl cursor-pointer hover:bg-white/5 group">
                    <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-[#80ff00]/20 transition-all">
                      <User className="h-5 w-5 text-white/40 group-hover:text-[#80ff00]" />
                    </div>
                    <span className="font-black text-white/60 group-hover:text-white uppercase italic text-xs tracking-widest">Settings</span>
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator className="bg-white/5 mx-2 my-2" />
                <DropdownMenuItem onClick={handleLogout} className="flex items-center gap-4 p-4 rounded-2xl cursor-pointer hover:bg-red-500/10 group">
                  <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-red-500/20 transition-all">
                    <LogOut className="h-5 w-5 text-white/40 group-hover:text-red-500" />
                  </div>
                  <span className="font-black text-white/60 group-hover:text-red-500 uppercase italic text-xs tracking-widest">Logout</span>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="fixed inset-0 z-[105] bg-[#050505]/95 backdrop-blur-3xl lg:hidden flex flex-col p-6 pt-24"
          >
            <div className="grid gap-3">
              <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.4em] mb-4 italic px-4">Vault Navigation</p>
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                ...
                >
                  ...
                  >
                    <div className={`p-3 rounded-xl ${pathname === link.href ? 'bg-[#80ff00]/20' : 'bg-white/5'}`}>
                      <link.icon className={`h-5 w-5 ${pathname === link.href ? 'text-[#80ff00]' : 'text-white/20'}`} />
                    </div>
              ...
              <div className="p-6 rounded-[2rem] bg-white/5 border border-white/5 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <p className="text-white/20 text-[8px] font-black uppercase truncate tracking-[0.2em]">{user?.email}</p>
                    </div>
                  </div>
                  <ThemeSwitcher />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    ...
                    variant="outline"
                    className="h-12 rounded-xl bg-white/5 border-white/10 text-white/60 font-black uppercase italic text-[10px] tracking-widest hover:text-white"
                  >
                    Settings
                  </Button>
                  <Button
