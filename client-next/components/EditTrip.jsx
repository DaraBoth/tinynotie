...
export function EditTrip({ open, onClose, groupId, trip, members = [], currency = '$' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const desc = eachMember ? `Each member: ${spend}` : description;
    ...
    return (
      <Sheet open={open} onOpenChange={onClose}>
        <SheetContent side="right" title="Add Trip" description="No members yet">
          ...
          <SheetBody>
            <p className="text-sm text-muted-foreground">You need to add at least one member before creating a trip.</p>
          </SheetBody>
          <SheetFooter>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <SheetBody>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="spend" type="number" min="0.01" step="0.01" placeholder="0.00"
                  value={spend} onChange={(e) => setSpend(e.target.value)} className="h-11 text-lg font-semibold pl-8" />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button type="button" variant="outline" size="sm" className="flex-1 gap-1.5 h-10"
                  onClick={() => applyChip(1)} disabled={chip === null && !custom}>
                  <Plus className="h-3.5 w-3.5" />
                  Add {chip !== null ? `${currency}${chip.toLocaleString()}` : custom ? `${currency}${custom}` : ''}
                </Button>
                <Button type="button" variant="outline" size="sm"
                  className="flex-1 gap-1.5 h-10 text-red-400 hover:text-red-400 border-red-400/30 hover:bg-red-400/10"
                  onClick={() => applyChip(-1)} disabled={chip === null && !custom}>
            ...
            <button type="button" onClick={() => setEachMember((p) => !p)}
              className="w-full flex items-center gap-3 cursor-pointer select-none p-3.5 rounded-xl border border-border/40 bg-muted/30 hover:bg-muted/50 transition-colors text-left">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Total = amount × selected members</p>
              </div>
            </button>
              ...
              <div className="flex flex-wrap gap-2 p-3 rounded-xl border border-border/40 bg-muted/20">
                {members.map((m) => (
                  <button key={m.id} type="button" onClick={() => toggleMember(m.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${selectedMemberIds.includes(m.id)
            ...
            <div className="space-y-1.5">
              ...
              <Input id="description" placeholder="e.g. Split evenly, includes drinks"
